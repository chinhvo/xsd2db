    ...
    public abstract class ScriptBasedDataSchemaAdapter : DataSchemaAdapter
    {
        ...
        private string ConvertDataTableToXML(DataTable dtData)
        {
            ...
            try
            {
                ...
                dsData.Tables[0].TableName = dtData.TableName;
        ...
        public void SaveToDB(string dataSetName, DataSet dataset)
        {
            using (var conn = GetConnection(dataSetName) as SqlConnection)
            {
                conn.Open();
                using (SqlBulkCopy bulkCopy = new SqlBulkCopy(conn))
                {
                    var myDataTable = dataset.Tables[0];
